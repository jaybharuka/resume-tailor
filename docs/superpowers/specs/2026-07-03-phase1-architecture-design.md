...
The orchestrator resolves the provider adapter, renders the prompt, calls the model, validates the response, and logs the call (§7, `llm_calls`). Provider adapters implemented in Phase 1: **NVIDIA** (primary — OpenAI-compatible client against `https://integrate.api.nvidia.com/v1`, model configurable — e.g. `z-ai/glm-5.2` — key read from `NVIDIA_API_KEY` env var, never hardcoded, with rate-limit/5xx/timeout errors classified as retryable and auth/4xx errors failing fast without retrying). **Gemini**, Claude, and OpenAI adapters are all stubbed with the same interface in Phase 1, wired in a later phase. (Originally Gemini was planned as the primary provider, reusing this repo's known-good backoff/rate-limit pattern; the project owner changed this mid-implementation once an NVIDIA API key was provisioned for this project — the backoff/jitter pattern was generalized into a shared helper so it isn't tied to either provider.)
...
1. Retries the *same* provider once with backoff (via the shared backoff/jitter helper, usable by any provider adapter).
...
Both items below are DB-hygiene gaps in the already-approved Task 4 schema. Neither is a live bug today (single-user local tool, no concurrent sync paths, nothing deletes `prompt_versions` rows yet), so they are tracked together to be addressed as one small migration in a later phase rather than reopening Task 4 mid-plan.

- **`llm_calls.prompt_version_id` cascade policy.** Task 4's implementation (post-review) applies `ondelete="CASCADE"` to every foreign key, per an explicit "CASCADE everywhere" decision made during Phase 1 implementation. This is correct for most of the schema, but it means deleting a `prompt_versions` row would cascade-delete every `llm_calls` row that references it — silently destroying the audit trail of what prompt actually produced a given AI-driven change, which conflicts with this project's "every modification should be explainable" principle (see the AI Design section above). When a later phase introduces prompt-version cleanup/deprecation, revisit this FK specifically: change it to `ondelete="RESTRICT"` (block deleting a prompt version while `llm_calls` still reference it) or nullify the FK on delete instead of cascading, so historical audit rows survive.
- **Missing `UniqueConstraint(task_type, version)` on `PromptVersion`.** Task 5's `PromptRegistry.sync_to_db()` relies on `(task_type, version)` being unique to stay idempotent, but the `PromptVersion` table (Task 4) has no DB-level uniqueness constraint enforcing that — uniqueness today is only an application-level `filter_by` check, so concurrent `sync_to_db` calls could theoretically race and insert duplicate rows. Add the constraint alongside the cascade-policy fix above.
